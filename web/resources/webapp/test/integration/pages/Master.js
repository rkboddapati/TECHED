				...
				actions : {
							...
							success : function (aEntityData) {
								this.waitFor({
									...
									success : function (oList) {
										var sCurrentId,
											aItemsNotInTheList = aEntityData.filter(function (oObject) {
												return !oList.getItems().some(function (oListItem) {
													return oListItem.getBindingContext().getProperty("PURCHASEORDERID") === oObject.PURCHASEORDERID;
												});
											});

										if (!aItemsNotInTheList.length) {
											// Not enough items all of them are displayed so we take the last one
											sCurrentId = aEntityData[aEntityData.length - 1].PURCHASEORDERID;
										} else {
											sCurrentId = aItemsNotInTheList[0].PURCHASEORDERID;
										}
										...
										oCurrentItem.bindingPath = "/" + oList.getModel().createKey("POHeader", {
											PURCHASEORDERID : sCurrentId
										});
					...
					iRememberTheListItem : function (oListItem) {
						...
						this.getContext().currentItem = {
							...
							id: oBindingContext.getProperty("PURCHASEORDERID"),
							title: oBindingContext.getProperty("PURCHASEORDERID")
						};
					}
				},
				...
				assertions : {
					...
					theListHeaderDisplaysZeroHits : function () {
						return this.waitFor({
							viewName : sViewName,
							id : "page",
							matchers : new PropertyStrictEquals({name : "title", value : "POHeader (0)"}),
							success : function () {
								Opa5.assert.ok(true, "The list header displays 'POHeader (0)'");
							},
					...
					theListShouldBeSortedAscendingOnName : function () {
						return this.theListShouldBeSortedAscendingOnField("PURCHASEORDERID");
					},
					...
					theListShouldHaveAllEntries : function () {
						...
						// retrieve all POHeader to be able to check for the total amount
						...
						return this.waitFor({
							...
							success : function (oList) {
								...
								this.waitFor({
									id : "page",
									viewName : sViewName,
									matchers : new PropertyStrictEquals({name : "title", value : "POHeader (" + iExpectedLength + ")"}),
									success : function () {
